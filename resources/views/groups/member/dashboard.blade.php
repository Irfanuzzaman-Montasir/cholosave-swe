        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold custom-font text-gray-900">
                    ...
                    {{ $group->group_name }} - Member Dashboard
                </h1>
                <p class="text-gray-600 mt-1">Welcome back! Here's your personal progress overview.</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto animate-fade-in">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div class="metric-card bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-gray-600">Your Savings Progress</p>
                            <p class="text-2xl font-bold text-gray-900">৳{{ number_format($availableSavings, 2) }}</p>
                            <div class="mt-2 space-y-1">
                                <div class="flex justify-between text-xs">
                                    <span class="text-gray-500">Total Saved:</span>
                                    <span class="font-medium text-blue-600">৳{{ number_format($personalSavings, 2) }}</span>
                                </div>
                                <div class="flex justify-between text-xs">
                                    <span class="text-gray-500">Withdrawn:</span>
                                    <span class="font-medium text-orange-600">৳{{ number_format($withdrawAmount, 2) }}</span>
                                </div>
                            </div>
                        </div>
                ...
                <div class="metric-card bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-gray-600">Installments Completed</p>
                            <p class="text-2xl font-bold text-gray-900">{{ $installmentsCompleted }}/{{ $group->time_period }}</p>
                            <p class="text-xs text-gray-500 mt-1">{{ number_format(($installmentsCompleted / $group->time_period) * 100, 1) }}% completed</p>
                            <p class="text-xs text-gray-500">Next due: {{ $nextPaymentDue->format('M d, Y') }}</p>
                        </div>
                ...
                <div class="metric-card bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-1">{{ $timeRemainingText }} left</p>
                            <div class="mt-2">
                                <div class="flex items-center">
                                    <div class="w-full bg-gray-200 rounded-full h-2 mr-2">
                                        @php
                                            $timeProgress = $group->time_period > 0 ? (($group->time_period - $timeRemaining) / $group->time_period) * 100 : 0;
                                        @endphp
                                        <div class="bg-orange-500 h-2 rounded-full" style="width: {{ min(100, $timeProgress) }}%"></div>
                                    </div>
                                    <span class="text-xs text-gray-500">{{ number_format($timeProgress, 1) }}%</span>
                                </div>
                            </div>
                        </div>
                ...
                <div class="metric-card bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-medium text-gray-600">Group Goal Progress</p>
                            <p class="text-2xl font-bold text-gray-900">৳{{ number_format($totalGroupSavings, 2) }}</p>
                            <div class="mt-2">
                                <div class="flex items-center">
                                    <div class="w-full bg-gray-200 rounded-full h-2 mr-2">
                                        <div class="bg-purple-500 h-2 rounded-full" style="width: {{ min(100, $groupGoalProgress) }}%"></div>
                                    </div>
                                    <span class="text-xs text-gray-500">{{ number_format($groupGoalProgress, 1) }}%</span>
                                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-semibold text-gray-900">Your Payment History</h3>
                        <span class="text-sm text-gray-500">Last 6 Months</span>
                    </div>
                    <div class="h-64 flex items-end justify-between space-x-2">
                        @foreach($paymentHistory as $month => $amount)
                ...
                <!-- Personal Goal Progress Meter -->
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-6">Your Savings Overview</h3>
                    <div class="text-center mb-6">
                        <div class="relative inline-block">
                            <svg class="progress-ring w-32 h-32" viewBox="0 0 120 120">
                                ...
                                <circle class="progress-ring-circle" stroke="#3b82f6" stroke-width="12" fill="transparent" r="48" cx="60" cy="60" 
                                        stroke-dasharray="{{ 2 * pi() * 48 }}" 
                                        stroke-dashoffset="{{ 2 * pi() * 48 * 0.75 }}"/>
                            </svg>
                            <div class="absolute inset-0 flex items-center justify-center">
                                <div class="text-center">
                                    <span class="text-3xl font-bold text-gray-900">৳{{ number_format($availableSavings, 0) }}</span>
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-medium text-gray-700">Available Savings</span>
                            <span class="text-lg font-bold text-gray-900">৳{{ number_format($availableSavings, 2) }}</span>
                        </div>
                        
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-medium text-gray-700">Total Saved</span>
                            <span class="text-lg font-bold text-blue-600">৳{{ number_format($personalSavings, 2) }}</span>
                        </div>
                        
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-medium text-gray-700">Withdrawn</span>
                            <span class="text-lg font-bold text-orange-600">৳{{ number_format($withdrawAmount, 2) }}</span>
                        </div>
                        
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-medium text-gray-700">Loan Due</span>
                            <span class="text-lg font-bold text-red-600">৳{{ number_format($loanDue, 2) }}</span>
                        </div>
                    </div>
                    
                    <div class="mt-6 pt-4 border-t border-gray-200">
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-gray-600">Financial Status:</span>
                            @if($loanDue > 0)
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-600">There are currently no active polls for this group.</p>
                    </div>
                @endif
            </div>
        </div>
